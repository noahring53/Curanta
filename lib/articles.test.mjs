...
import {
  ...
  buildArticleMessages, getSourceArticle, getSources,
...
const srcA = toSourcePayload({ title: 'Vote passes 5-2', source: 'Gazette', text: para(300), url: 'https://g.com/a' });
const srcB = toSourcePayload({ title: 'Council green-lights bond', source: 'Tribune', text: para(300), url: 'https://t.com/b' });
const multi = buildArticleMessages({ angle: 'impact on residents', sources: [srcA, srcB] });
ok('lists every source with an index', /SOURCE 1 of 2/.test(multi.user) && /SOURCE 2 of 2/.test(multi.user));
ok('instructs synthesis, not N mini-articles', /Synthesise ONE coherent article/.test(multi.user));
ok('both source URLs reach the model', multi.user.includes('https://g.com/a') && multi.user.includes('https://t.com/b'));
const single = buildArticleMessages({ angle: 'x', sources: [srcA] });
ok('a single source uses the plain SOURCE label (no synthesis note)',
  single.user.includes('SOURCE\n') && !/Synthesise ONE/.test(single.user));

let mfetches = 0;
const multiFetch = async (url) => { mfetches++; return { title: 'S', source: 'X', text: para(300), url }; };
const gotMulti = await getSources(['https://m.com/1', 'https://m.com/2', 'https://m.com/1'], { fetchArticle: multiFetch });
ok('getSources dedupes identical URLs before fetching', gotMulti.sources.length === 2);
const partial = await getSources(['https://ok.com/1', 'https://bad.com/x'], {
  fetchArticle: async (url) => { if (url.includes('bad')) throw new Error('blocked'); return { title: 'ok', source: 'Y', text: para(300), url }; },
});
ok('getSources keeps good sources and records failures', partial.sources.length === 1 && partial.failures.length === 1);
let allBad = null;
try { await getSources(['https://x/1'], { fetchArticle: async () => { throw new Error('nope'); } }); }
catch (e) { allBad = e; }
ok('getSources throws when every source fails', allBad !== null);
