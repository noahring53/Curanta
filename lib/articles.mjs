...
export function registerArticleRoutes(app, deps = {}) {
  const {
    ...
    loadPrompt = async () => null,   // → { prompt, name } | null
  ...
  app.post('/api/articles/generate', generateLimiter, async (req, res) => {
    const {
      ...
      notes = '',
      promptId = '',
      promptText = '',   // only used when no server-side prompt store exists
      mode = 'news',
      refresh = false,
      stream = false,
      userId = '',
    ...
    // The master prompt is read from the store by ID. promptText is the fallback
    // for installs with no database — never the primary path, and bounded so an
    // unbounded blob can't be pushed through the client.
    let masterPrompt = '';
    let promptName = '';
    if (promptId) {
      const row = await loadPrompt(promptId, userId, authToken);
      if (row) { masterPrompt = row.prompt || ''; promptName = row.name || ''; }
    }
    if (!masterPrompt && promptText) masterPrompt = String(promptText).slice(0, 8000);
    ...
    console.log(
      `[articles] generate mode=${mode} prompt="${promptName || (masterPrompt ? '(inline)' : 'none')}" ` +
